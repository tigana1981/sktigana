...
// --- Real-Debrid (voliteľné) ---
// Nastav v prostredí RD_TOKEN (API token z real-debrid.com/apitoken)
// Keď je RD_TOKEN prázdny, addon sa správa pôvodne (vracia infoHash a Stremio použije torrent engine).
...
function buildMagnet(infoHash, name) {
    const dn = encodeURIComponent(name || infoHash);
    return `magnet:?xt=urn:btih:${infoHash}&dn=${dn}`;
}

function isProbablyVideo(filename = "") {
    return /\.(mkv|mp4|avi|mov|wmv|m2ts|ts|webm|mpg|mpeg)$/i.test(filename);
}

async function rdRequest(method, path, data) {
    if (!RD_TOKEN) throw new Error("RD_TOKEN nie je nastavený");
    ...
    const res = await axios({
        ...
        headers: {
            ...
            "User-Agent": "SKTorrent-Stremio-Addon"
        },
        timeout: 15000
    });
    return res.data;
...
async function rdResolveToUrl(infoHash, displayName) {
    // 1) pridaj magnet
    const magnet = buildMagnet(infoHash, displayName);
    const added = await rdRequest("post", "/torrents/addMagnet", { magnet });
    const torrentId = added?.id;
    if (!torrentId) throw new Error("Real-Debrid nevrátil torrent id");

    // 2) vyber súbory (all)
    try {
        await rdRequest("post", `/torrents/selectFiles/${torrentId}`, { files: "all" });
    } catch (e) {
        // 202: action already done – je ok
    }

    // 3) počkaj, kým sa objavia linky
    let info = null;
    for (let i = 0; i < 6; i++) {
        info = await rdRequest("get", `/torrents/info/${torrentId}`);
        if (info?.links?.length) break;
        await sleep(700);
    }
    const links = info?.links || [];
    if (!links.length) {
        // uprac zo zoznamu, aby to nezostalo visieť
        try { await rdRequest("delete", `/torrents/delete/${torrentId}`); } catch (_) {}
        return null;
    }

    // 4) nájdi prvý video súbor a zober zodpovedajúci link
    //    Real-Debrid vracia 'files' (metadáta) a 'links' (hoster linky). Poradie zvyčajne sedí.
    let idx = 0;
    if (Array.isArray(info?.files) && info.files.length === links.length) {
        const videoIdx = info.files.findIndex(f => isProbablyVideo(f.path || f.filename || ""));
        if (videoIdx >= 0) idx = videoIdx;
    }
    const hosterLink = links[idx] || links[0];

    // 5) unrestrict -> dostaneš priamy https link
    const unrestricted = await rdRequest("post", "/unrestrict/link", { link: hosterLink });
    const direct = unrestricted?.download || unrestricted?.link;

    // 6) (voliteľné) zmaž torrent z queue, aby sa ti neplnila
    try { await rdRequest("delete", `/torrents/delete/${torrentId}`); } catch (_) {}
...
async function toStream(t) {
    ...
    // Ak je nastavený Real-Debrid token, skús vrátiť priamy https stream (url) namiesto infoHash.
    // Keď RD nie je dostupný / nie je cache, fallbackne to pôjde cez infoHash.
    let rdUrl = null;
    if (RD_TOKEN) {
        try {
            rdUrl = await rdResolveToUrl(infoHash, cleanedTitle);
        } catch (e) {
            console.warn("[WARN] Real-Debrid resolve zlyhal:", e?.message || e);
        }
    }
    ...
    if (rdUrl) stream.url = rdUrl;
    else stream.infoHash = infoHash;
...
console.log(`🚀 SKTorrent addon beží na porte ${port} (manifest: /manifest.json)`);
